...
class SlackSingleGame(object):
    ...
    def create(self):
        r.post('http://www.yaiir.com/games', json=self._dictify())
        return self

    def delete(self):
        r.delete('http://www.yaiir.com/games/{}'.format(self.game_id))
        return self
...
class SlackPlayerList(object):
    def __init__(self, days=None):
        query_url = 'http://www.yaiir.com/players'
        if days:
            query_url = 'http://www.yaiir.com/players?days={}'.format(days)
...
class SlackChanges(object):
    def __init__(self):
        raw_data = r.get('http://www.yaiir.com/players').json()
